...
export interface Hospital {
  id: string;
  name: string;
  type: string; // 'Government Medical College', 'Private Super Speciality', 'Trust Hospital', 'Trauma Care Center'
  address: string;
  latitude: number;
  longitude: number;
  phone: string;
...
export interface HospitalSpecialist {
  id: string;
  hospital_id: string;
  specialty: string; // 'Neurosurgeon' | 'Orthopedic' | 'Cardiologist' | 'Trauma Specialist' | 'General Surgeon' | 'Anesthetist'
  doctor_name: string;
...
export interface EmergencyIncident {
  ...
  emergency_type: string; // 'Road Accident' | 'Severe Bleeding' | 'Cardiac Emergency' | 'Breathing Difficulty' | 'Trauma' | 'Other'
...
export interface EmergencyEventMongo {
  _id?: string;
  incident_id: string;
  event_type: string;
...
export interface AmbulanceLocationHistoryMongo {
  _id?: string;
  ambulance_id: string;
  incident_id?: string;
  latitude: number;
...
export interface ActivityLogMongo {
  _id?: string;
  user_id?: string;
  role: UserRole;
  action: string;
...
// Resource-aware Matching Details
